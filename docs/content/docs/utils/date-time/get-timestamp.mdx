...
- Exported as `getTimestamp` (lowercase `s`), separate from [`Chronos.getTimeStamp()`](https://chronos.nazmul-nhb.dev/docs/chronos/get-set/get-timestamp) which returns timestamp in milliseconds.
