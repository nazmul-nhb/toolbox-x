---
title: Get Timestamp
description: Get Timestamp — Toolbox-X Documentation.
---
...
- Exported as `getTimestamp` (lowercase `s`), separate from [`Chronos.getTimeStamp()`](https://chronos-date.vercel.app/docs/chronos/get-set/get-timestamp) which returns timestamp in milliseconds.
