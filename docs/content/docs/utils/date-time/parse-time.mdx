...
description: Parse Time Value — Toolbox-X Documentation.
---
...
```typescript
import { parseMSec } from "toolbox-x/date";
```
...
import { parseMSec } from "toolbox-x/date";
import type { TimeWithUnit } from "toolbox-x/types";
...
<Callout type="info" title="Note">
  This function is designed for parsing time into ms/sec, not absolute timestamps. For date/time parsing and manipulation, consider using [Chronos](https://chronos-date.vercel.app/docs/chronos) class or other [date utilities](/docs/utils).
</Callout>
