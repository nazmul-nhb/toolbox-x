...
<Callout type="info" title="Note">
  This function is designed for parsing time into ms/sec, not absolute timestamps. For date/time parsing and manipulation, consider using [Chronos](https://chronos.nazmul-nhb.dev/docs/chronos) class or other [date utilities](/docs/utils).
</Callout>
