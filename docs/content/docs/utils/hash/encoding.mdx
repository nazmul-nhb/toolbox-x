...
console.log([...bytes]); // Converting to a normal array for logging
```
...
console.log([...hash]); // Converting to a normal array for logging
```
...
```typescript pg
import { hmacSha256, utf8ToBytes } from 'toolbox-x/hash';
const key = utf8ToBytes('secret');
const msg = utf8ToBytes('Hello');
const hmac = hmacSha256(key, msg);

console.log([...hmac]); // Converting to a normal array for logging
```
...
console.log([...result]); // Converting to a normal array for logging
```
...
console.log([...be]); // Converting to a normal array for logging
```
...
// Basic conversion
const hex = '12abff00';
const bytes = [...hexToBytes(hex)]; // Converting to a normal array for logging
// [18, 171, 255, 0]

// With spaces
const spacedHex = '48 65 6c 6c 6f';
const spacedBytes = [...hexToBytes(spacedHex)]; // Converting to a normal array for logging
// [72, 101, 108, 108, 111]

// Empty input
const emptyBytes = [...hexToBytes('')]; // Converting to a normal array for logging
// []

// Invalid input returns empty array
const invalidBytes = [...hexToBytes('invalid!')]; // Converting to a normal array for logging
// []
const oddBytes = [...hexToBytes('123')]; // Converting to a normal array for logging
// []

// Cryptographic hash decoding
const sha256Hex = '2cf24dba5fb0a30e26e83b2ac5b9e29e1b161e5c1fa7425e73043362938b9824';
const hashBytes = [...hexToBytes(sha256Hex)]; // Converting to a normal array for logging
// [28, 242, 77, 186, ...]
...
- **Automatic validation** - Uses [`isHexString`](/docs/guards/misc/string-format#ishexstring) to validate input
...
- [**isHexString**](/docs/guards/misc/string-format#ishexstring) - Input validation type guard
