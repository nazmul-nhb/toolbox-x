...
description: Get Time Zone Details — Toolbox-X Documentation.
---
...
```typescript
import { getTimeZoneDetails } from "toolbox-x/date";
```
...
```typescript
import { getNativeTimeZoneId } from "toolbox-x/date";
```
...
```typescript
import { getTimeZoneIds } from "toolbox-x/date";
```
