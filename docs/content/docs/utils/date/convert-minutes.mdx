...
description: Convert Minutes into Formatted Time String — Toolbox-X Documentation.
---
...
```typescript pg
import { convertMinutesToTime, getTimeFromMinutes } from "toolbox-x/date";
...
```ts pg
import { convertMinutesToTime } from "toolbox-x/date";
