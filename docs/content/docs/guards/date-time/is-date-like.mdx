...
- Supports multiple popular date libraries:
  ...
  - `JS-Joda`
  - `Temporal`
  - [`Chronos`](https://chronos-date.vercel.app/docs/chronos)
...
```ts
import { isDateLike } from 'toolbox-x/guards';
...
- [`isDateString`](./is-date-string) - Checks if a value is a date string.