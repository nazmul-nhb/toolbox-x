...
```ts pg
import { isTimeWithUnit } from 'toolbox-x/guards';
import { parseMSec } from 'toolbox-x/date';
import type { TimeWithUnit } from 'toolbox-x/types/date';
...
- [`parseMSec`](/docs/utils/date-time/parse-time#parsemsec) - Parse time strings to milliseconds/seconds
