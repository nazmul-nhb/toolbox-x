...
<Callout type="idea" title="Important Note">
  - You should not use this method explicitly unless you have a specific use case that requires it.
  - Instead, consider using the `{ fuzzy: true }` option in the [`findOne`](./find-one), [`findAll`](./find-all), [`findOneAsync`](./find-one-async), and [`findAllAsync`](./find-all-async) methods to enable fuzzy search.
</Callout>
