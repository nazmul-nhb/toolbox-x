...
<Callout type="idea" title="Important Note">
  ...
  - Instead, consider using the `{ forceBinary: true }` option in the [`findOne`](./find-one), [`findAll`](./find-all), [`findOneAsync`](./find-one-async), and [`findAllAsync`](./find-all-async) methods to enable binary search.
</Callout>
