...
- [binarySearch()](./binary-search) — Performs binary search on sorted array using a custom key selector function.
- [clearCache()](./clear-cache) — Manages and clears cached search results.
- [findAll()](./find-all) — Finds all items matching specified criteria.
- [findAllAsync()](./find-all-async) — Asynchronously retrieves all items matching the specified criteria.
- [findOne()](./find-one) — Finds the first item matching specified criteria.
- [findOneAsync()](./find-one-async) — Asynchronously finds the first matching item.
- [fuzzySearch()](./fuzzy-search) — Performs a fuzzy search matching characters in sequence.
